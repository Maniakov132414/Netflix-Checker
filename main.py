...
DISCORD_WEBHOOK_USERNAME = "Netflix Checker"
...
def _render_update_notice(latest_version, github_url, discord_url):
    ...
    print("")
    print(divider)
    print(f"Phiên bản hiện tại: {APP_VERSION} | Phiên bản mới nhất: {latest_version}")
    print(f"Tải bản cập nhật tại: {github_url}")
    print(f"Tham gia Discord: {discord_url}")
    print(divider)
    print("")
...
def load_config():
    ...
    if os.path.exists(config_yaml_path):
        if yaml is None:
            print("Cảnh báo: Chưa cài đặt PyYAML. Hãy chạy lệnh: pip install -r requirements.txt")
        ...
        try:
            ...
            print("Cảnh báo: File config.yml không hợp lệ. Đang tạo lại file mặc định.")
...
def print_config_summary(config, config_source):
    ...
    print("Cấu hình đang sử dụng")
    print(f"- Tệp cấu hình: {config_source}")
    print(f"- Các trường nội dung bật: {', '.join(enabled_txt) if enabled_txt else 'không có'}")
    print(f"- Link NFToken: {nftoken_mode}")
    print(f"- Biểu tượng cảm xúc (Emoji): {add_emojis_mode}")
    print(f"- Webhook: {'BẬT' if webhook_cfg.get('enabled') else 'TẮT'} (chế độ: {webhook_cfg.get('mode', 'full')})")
    print(f"- Telegram: {'BẬT' if telegram_cfg.get('enabled') else 'TẮT'} (chế độ: {telegram_cfg.get('mode', 'full')})")
    print(f"- Hiển thị: chế độ={display_cfg.get('mode', 'log')}")
    print(f"- Số lần thử lại khi lỗi mạng: {retry_attempts}")
    print(f"- Thời gian chờ yêu cầu (giây): {request_timeout_seconds}")
    print(f"- Trang dự phòng (/YourAccount): {'BẬT' if fallback_account_page else 'TẮT'}")
    print(f"- Thử lại trang thông tin thiếu: {'BẬT' if retry_incomplete_info else 'TẮT'}")
    print(f"- Tạo NFToken cho tài khoản free: {'BẬT' if nftoken_for_free else 'TẮT'}")
    print("")
...
# === HÀM render_simple_dashboard - THÊM ETA VÀ RATE ===
def render_simple_dashboard(counts, plan_counts, plan_labels, cookies_left, cookies_total, colored=True, eta="--", rate="0.00"):
    ...
    print(BANNER)
    print(color_text("NetflixChecker - Chế độ Đơn giản", title_color, colored))
    print(
        f"{color_text('Tiến độ:', title_color, colored)} "
    ...
    print("")
    print(color_text("Thông tin", section_color, colored))
    print(f"{color_text('Hợp lệ   :', label_color, colored)} {color_text(str(valid), good_color, colored)}")
    print(f"{color_text('Ngon     :', label_color, colored)} {color_text(str(counts['hits']), good_color, colored)}")
    print(f"{color_text('Miễn phí :', label_color, colored)} {color_text(str(counts['free']), free_color, colored)}")
    print(f"{color_text('Hỏng     :', label_color, colored)} {color_text(str(counts['bad']), bad_color, colored)}")
    print(f"{color_text('Trùng    :', label_color, colored)} {color_text(str(counts['duplicate']), value_color, colored)}")
    print(f"{color_text('Bị giữ   :', label_color, colored)} {color_text(str(counts['on_hold']), hold_color, colored)}")
    print(f"{color_text('Lỗi      :', label_color, colored)} {color_text(str(counts['errors']), bad_color, colored)}")
...
def build_account_detail_lines(config, info, is_subscribed, output_filename=None, use_emojis=False, include_country_flag=True):
    ...
    labels = [
        ...
        ("phone", "Số điện thoại"),
        ("quality", "Chất lượng"),
        ("max_streams", "Số luồng"),
        ("plan_price", "Giá gói"),
        ("hold_status", "Trạng thái giữ"),
        ("extra_members", "Thành viên bổ sung"),
        ("email_verified", "Email Verified"),
        ("membership_status", "Trạng thái thành viên"),
        ("profiles", "Các hồ sơ"),
        ("user_guid", "ID người dùng"),
    ]
    ...
    for key, label in labels:
        ...
        if txt_fields.get(key, True):
            ...
            if key == "profiles" and info.get("profileCount"):
                rendered_label = f"Các hồ sơ ({info['profileCount']})"
            if use_emojis:
...
def format_cookie_file(info, cookie_content, config, is_subscribed, nftoken_data=None):
    ...
    if is_subscribed and nftoken_mode != "false" and usable_nftoken:
        ...
        lines.append("")
        lines.append("Chi tiết về NFToken :👇")
        lines.append("")
        ...
        if isinstance(nftoken_data, dict) and nftoken_data.get("expires_at_utc"):
            valid_till_label = decorate_notification_label("Có hiệu lực đến", enabled=txt_emojis_enabled)
...
def build_notification_details(config, info, is_subscribed, output_filename):
    status = "Đã đăng ký" if is_subscribed else "Working (No Subscription)"
    if not is_subscribed:
        _, normalized_plan_label = derive_plan_info(info, is_subscribed)
        profiles_value = normalize_output_value(info.get("profilesDisplay"))
        profile_count = info.get("profileCount")
        profile_label = f"Các hồ sơ ({profile_count})" if profile_count else "Các hồ sơ"
        lines = [
            f"Tên: {normalize_output_value(info.get('accountOwnerName'))}",
            f"Email: {normalize_output_value(info.get('email'))}",
            f"Quốc gia: {format_country_with_flag(info.get('countryOfSignup'))}",
            f"Gói: {normalize_output_value(normalized_plan_label)}",
    ...
    return [f"Thông tin: {status}"] + lines
...
def decorate_notification_label(label, enabled=True):
    ...
    if normalized.startswith("Các hồ sơ ("):
        normalized = "Các hồ sơ"
...
def build_discord_full_message(config, info, is_subscribed, output_filename, nftoken_data=None, use_emojis=True):
    ...
    if links:
        ...
        if expiry_unix is not None:
            lines.append(f"**{decorate_notification_label('Có hiệu lực đến', enabled=use_emojis)}:** <t:{expiry_unix}:R>")
    lines.extend(
        [
            "",
...
def build_discord_nftoken_message(info, nftoken_data, nftoken_mode, use_emojis=True):
    ...
    if links:
        ...
        lines.append(f"**{decorate_notification_label('Quốc gia', enabled=use_emojis)}:** {country_display}")
        lines.append("")
        for label, link in links:
            rendered_label = render_nftoken_link_label(label, use_emojis)
            lines.append(f"**{rendered_label}:** [Click here]({link})")
        if isinstance(nftoken_data, dict) and nftoken_data.get("expires_at_utc"):
            ...
            if expiry_unix is not None:
                ...
                lines.append(f"**{decorate_notification_label('Có hiệu lực đến', enabled=use_emojis)}:** {nftoken_data['expires_at_utc']}")
    else:
...
def build_telegram_full_message(config, info, is_subscribed, output_filename, nftoken_data=None, use_emojis=True):
    ...
    if links:
        ...
        if isinstance(nftoken_data, dict) and nftoken_data.get("expires_at_utc"):
            lines.append(
                f"<b>{_escape_html(decorate_notification_label('Có hiệu lực đến', enabled=use_emojis))}:</b> "
...
def build_telegram_nftoken_message(info, nftoken_data, nftoken_mode, use_emojis=True):
    ...
    if links:
        ...
        lines.append(f"<b>{_escape_html(decorate_notification_label('Quốc gia', enabled=use_emojis))}:</b> {_escape_html(country_display)}")
        lines.append("")
        for label, link in links:
            rendered_label = render_nftoken_link_label(label, use_emojis)
            lines.append(f'<b>{_escape_html(rendered_label)}:</b> <a href="{_escape_html(link)}">Click here</a>')
...
# === HÀM check_cookies - CÓ ETA + TỰ ĐỘNG GIỚI HẠN THREAD ===
def check_cookies(num_threads=30, config=None):
    ...
    def get_next_proxy(used_proxy_indices):
        ...
        with proxy_cursor_lock:
            for _ in range(len(proxies)):
                rotated_index = proxy_cursor[0] % len(proxies)
                proxy_cursor[0] = (proxy_cursor[0] + 1) % len(proxies)
                if rotated_index not in used_proxy_indices:
                    chosen_index = rotated_index
                    break
            else:
    ...
    def handle_result(info, netscape_content, cookie_path, cookie_file, is_subscribed, cookie_dict, remove_source=True):
        ...
        if is_subscribed:
            ...
            filename = f"{country}_Hachimi_{random_suffix}.txt"
        else:
    ...
    try:
        ...
        if display_mode == "simple":
            print(color_text("\nĐang dừng... vui lòng chờ.", "\033[93m", True))
        else:
            print("\nĐang dừng... vui lòng chờ.")
        for t in threads:
            t.join(timeout=1)
        set_console_title("NetflixChecker - Đã dừng")
        return

    valid = counts["hits"] + counts["free"]
    set_console_title(
        f"NetflixChecker - Hoàn tất Hợp lệ {valid} Lỗi {counts['bad']} "
        f"Duplicate {counts['duplicate']} OnHold {counts['on_hold']} Errors {counts['errors']}"
    )

    # === XÓA FOLDER RỖNG (GIỐNG main_ui.py) ===
    ...
    if display_mode == "simple":
        ...
        line = "==================== Tổng Kết ===================="
        print(color_text("\n\n" + line, "\033[95m", True))
        print(color_text("Đã check   :", label_code, True), color_text(str(cookies_total), value_code, True))
        print(color_text("Ngon       :", label_code, True), color_text(str(counts["hits"]), good_code, True))
        print(color_text("Miễn Phí   :", label_code, True), color_text(str(counts["free"]), free_code, True))
        print(color_text("Hỏng       :", label_code, True), color_text(str(counts["bad"]), bad_code, True))
        print(color_text("Trùng      :", label_code, True), color_text(str(counts["duplicate"]), value_code, True))
...
def main():
    ...
    print("--------------------------------------------------------------------------------------------------")
    print("")
    print("            👉  Chào mừng! Sau khi bỏ cookies vào thư mục (cookies), hãy nhấn  👈")
    print("                              Enter nếu đã sẵn sàng bắt đầu!")
    input()

    # === QUÉT SUBFOLDER IM LẶNG (GIỐNG main_ui.py) ===
    initial_files = []
    for root, dirs, files in os.walk(cookies_folder):
        for f in files:
            if f.lower().endswith((".txt", ".json")) and not f.startswith("."):
    ...
    # === XÓA FOLDER RỖNG LẦN CUỐI (GIỐNG main_ui.py) ===
    if os.path.exists(cookies_folder):
        for root, dirs, files in os.walk(cookies_folder, topdown=False):
            for dir_name in dirs:
                full_path = os.path.join(root, dir_name)
